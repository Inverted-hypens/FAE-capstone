...
Flyrank Frontend AI Engineering capstone. Next.js app initialized — update this file as the stack evolves.
...
| Framework   | Next.js      | v16, App Router, `src/app/`                |
| UI          | React        | v19                                        |
| Styling     | Tailwind CSS | v4 via `@tailwindcss/postcss`              |
| State       | —            | not in use yet                             |
| API / Data  | —            | not in use yet                             |
| Testing     | —            | not in use yet                             |
| Linting     | ESLint       | v9, `eslint-config-next`                   |
| Package mgr | npm          | `package-lock.json`                        |
| Node        | 20.x         | `@types/node` ^20                          |
...
## Rules learned from FE-03 (vague vs. engineered prompting drill)

- Forms use react-hook-form + zod for validation — never uncontrolled
  inputs with manual useState/onChange handlers. (Round 1's hand-rolled
  state management produced zero validation; round 2's schema-based
  approach made every constraint explicit and testable.)
- Any "selected" or "active" UI state must have a non-color indicator
  (icon, border weight, shape change) in addition to color — color alone
  fails accessibility review even when the intent was there. (Missed in
  round 2's personality-trait selector, caught in review.)
- New components require a request for tests before being considered
  done — untested code should be flagged as incomplete, not shippable,
  regardless of how polished it looks. (Round 1 looked done; it wasn't.)
